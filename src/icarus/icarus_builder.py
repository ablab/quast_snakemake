...
def get_assemblies_data(reports, labels, icarus_dirpath, stdout_pattern, nx_marks):
    ...
    for label in labels:
        l = reports[label].get_field(reporting.Fields.TOTALLEN)
        contigs = reports[label].get_field(reporting.Fields.CONTIGS)
        n50 = reports[label].get_field(reporting.Fields.N50)
        if stdout_pattern:
            contig_stdout_fpath = stdout_pattern % label + '.stdout'
        ...
        for nx in nx_marks:
            assemblies_n50[label][nx] = reports[label].get_field(nx)
