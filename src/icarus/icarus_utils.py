...
from src.html_saver.html_saver import trim_ref_name
...
class Assembly:
    def __init__(self, contigs_fpath, label, aligned_blocks, min_visualizer_length=0):
...
class Assemblies:
    def __init__(self, contigs_fpaths, labels,
        ...
        for i, c_fpath in enumerate(contigs_fpaths):
            self.assemblies.append(
                Assembly(c_fpath, labels[i], lists_of_aligned_blocks[i],
...
def get_assemblies(contigs_fpaths, labels, lists_of_aligned_blocks, virtual_genome_size=None, find_similar=True):
    min_visualizer_length = 0

    assemblies = Assemblies(
        contigs_fpaths, labels,
        lists_of_aligned_blocks,
        virtual_genome_size,
