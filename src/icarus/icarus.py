...
from src.icarus.icarus_parser import parse_aligner_contig_report
from src.icarus.icarus_utils import *
...
import src.html_saver.html_saver as html_saver
from src.misc import ref_labels_by_chromosomes


def do(reports, contigs_fpaths, contig_report_fpath_pattern, output_dirpath, ref_fpath,
    ...
    for label, contigs_fpath in zip(labels, contigs_fpaths):
    ...
    if reference_chromosomes and lists_of_aligned_blocks:
        assemblies = get_assemblies(contigs_fpaths, labels, lists_of_aligned_blocks, virtual_genome_size, find_similar)
    if (assemblies or contigs_by_assemblies) and qconfig.create_icarus_html:
        icarus_html_fpath = js_data_gen(reports, assemblies, contigs_fpaths, labels, reference_chromosomes,
...
def js_data_gen(reports, assemblies, contigs_fpaths, labels, chromosomes_length, output_dirpath, structures_by_labels,
            ...
            for align in assembly.alignments:
                ...
                if align.similar:
                    if align.misassembled:
                        ...
                        similar_correct += 1
            reports[assembly.label].add_field(reporting.Fields.SIMILAR_CONTIGS, similar_correct)
            reports[assembly.label].add_field(reporting.Fields.SIMILAR_MIS_BLOCKS, similar_misassembled)
    ...
    main_data_dict['assemblies'] = ', '.join(labels)
    ...
    if chr_full_names and (len(chr_full_names) > 1 or qconfig.is_combined_ref):
        ...
        if is_unaligned_asm_exists:
            table_references['th_assemblies'] = True
        for chr in sorted(chr_full_names, key=natural_sort):
            ...
            table_references['references'].append(reference_dict)
        main_data_dict.update(table_references)
        html_saver.save_icarus_data(json_output_dir, table_references, 'table_references', as_text=False)
    else:
        if chr_full_names:
            one_reference_dict = dict()
            chr = chr_full_names[0]
            chr_link, chr_name, chr_genome, chr_size, tooltip = get_info_by_chr(chr, aligned_bases_by_chr, chr_sizes, contigs_fpaths,
                                                                                contig_names_by_refs, one_chromosome=True)
            one_reference_dict['alignment_link'] = chr_link
            one_reference_dict['ref_fpath'] = os.path.basename(ref_fpath)
            one_reference_dict['ref_fragments'] = str(num_contigs[chr])
            one_reference_dict['ref_size'] = format_long_numbers(chr_size)
            one_reference_dict['ref_gf'] = '%.3f' % chr_genome
            one_reference_dict['one_num_misassemblies'] = str(num_misassemblies[chr])
            icarus_links["links"].append(chr_link)
            icarus_links["links_names"].append(qconfig.icarus_link)
            html_saver.save_icarus_data(json_output_dir, one_reference_dict, 'menu_reference', as_text=False)
            main_data_dict.update(one_reference_dict)
