...
def correct_fasta(original_fpath, min_contig, corrected_fpath=None, is_reference=False):
    ...
    for first_line, seq in read_fasta(original_fpath):
        if not first_line:
...
def get_lengths_from_fasta(contigs_fpath, label):
    lengths = get_chr_lengths_from_fastafile(contigs_fpath).values()

    if not sum(l for l in lengths if l >= qconfig.min_contig):
        print_warning("Skipping %s because it doesn't contain contigs >= %d bp."
...
def add_lengths_to_report(lengths, reporting, label):
    if reporting:
        ## filling column "Assembly" with names of assemblies
        report = reporting.get(label)

        ## filling columns "Number of contigs >=110 bp", ">=200 bp", ">=500 bp"
        is_broken = False
        if qconfig.split_scaffolds:
            if label in qconfig.dict_of_broken_scaffolds or \
                            get_color_and_ls(label)[1] == secondary_line_style:
                is_broken = True
...
def correct_contigs(contigs_fpaths, corrected_dirpath, labels, reporting):
    ...
    if qconfig.draw_plots or qconfig.html_report:
        if not dict_color_and_ls:
            save_colors_and_ls(corrected_contigs_fpaths)
...
def broke_scaffolds(file_counter, labels, contigs_fpath, corrected_dirpath, logs):
    ...
    scaffold_counter = 0
    for scaffold_counter, (name, seq) in enumerate(read_fasta(contigs_fpath)):
    ...
    if contigs_counter > scaffold_counter + 1:
        write_fasta(broken_scaffolds_fpath, broken_scaffolds_fasta)
...
def parse_labels(line, contigs_fpaths):
    ...
    if len(labels) != len(contigs_fpaths):
        print_error('Number of labels does not match the number of files with contigs', 11, to_stderr=True)
        return []
    else:
...
def assert_file_exists(fpath, message=''):
    if not os.path.isfile(fpath):
        print_error("File not found (%s): %s" % (message, fpath), 2,
                     to_stderr=True)
...
#         except Exception, e:
#             print_error('can\'t open zip file: ' + str(e.message))
#             return False
#
#         names = zfile.namelist()
#         if len(names) == 0:
#             print_error('zip archive is empty')
#             return False
#
#         if len(names) > 1:
#             print_warning('zip archive must contain exactly one file. Using %s' % names[0])
#
...
def check_is_fasta_file(fname):
    ...
    if outer_ext not in ['.zip', '.gz', '.gzip', '.bz2', '.bzip2', '']:
        print_warning('Skipping %s because it is not a supported archive.' % fname)
        return False

    if fasta_ext not in ['.fa', '.fasta', '.fas', '.seq', '.fna']:
        print_warning('Skipping %s because it has not a supported extension.' % fname)
        return False
...
def call_subprocess(args, stdin=None, stdout=None, stderr=None,
    ...
    print_command_line(printed_args, indent, only_if_debug=only_if_debug)

    return_code = subprocess.call(args, stdin=stdin, stdout=stdout, stderr=stderr, env=env)

    if return_code != 0:
        print_debug(' ' * len(indent) + 'The tool returned non-zero.' +
        ...
        if os.path.isfile(fpath) and os.access(fpath, os.X_OK):
            ...
            if version_check != OLD_VERSION:
                if version_check == NOT_RECOMMENDED_VERSION:
                    print_warning('Version of installed %s differs from its version in QUAST package (%s). '
...
def safe_create(fpath, is_required=False):
    try:
        open(fpath, 'w').close()
    except IOError:
...
def compile_tool(name, dirpath, requirements, just_notice=False, only_clean=False, flag_suffix=None,
    ...
    if not all_required_binaries_exist(dirpath, requirements):
        ...
        # making
        print_info('Compiling ' + name + ' (details are in ' + make_logs_basepath +
                         '.log and make.err)')
        ...
        if return_code != 0 or not all_required_binaries_exist(dirpath, requirements):
            write_failed_compilation_flag(name, dirpath, failed_compilation_flag, just_notice=just_notice)
            return False
    return True
...
def check_dirpath(path, message="", exit_code=3):
    if not is_ascii_string(path):
        print_error('QUAST does not support non-ASCII characters in path.\n' + message, to_stderr=True, exit_with_code=exit_code)
    if ' ' in path:
        print_error('QUAST does not support spaces in paths.\n' + message, to_stderr=True, exit_with_code=exit_code)
    return True
...
def write_failed_compilation_flag(tool, tool_dirpath, failed_compilation_flag, just_notice=False):
...
def download_blast_binaries(filenames=None, only_clean=False):
    ...
    blast_dirpath = get_dir_for_download('blast', 'BLAST', filenames, only_clean=only_clean)
    ...
    for i, cmd in enumerate(required_files):
        blast_fpath = download_blast_binary(cmd)
        print_info()
        if not blast_fpath:
            return False
...
def download_file(url, fpath, tool_name, move_file=True):
    if not os.path.exists(fpath):
        print_info('Downloading %s (file: %s)...' % (tool_name, os.path.basename(fpath)))
        try:
        ...
        if move_file:
            ...
            print_info('%s successfully downloaded!' % tool_name)
    return fpath


def download_blast_binary(blast_filename):
...
def download_external_tool(fname, dirpath, tool, platform_specific=False, is_executable=False):
    ...
    if isfile(external_fpath):
        print_info('Copying ' + fname + ' from ' + external_fpath)
    ...
    if is_executable:
        if downloaded_fpath:
            ...
            print_error('Please try to install the tool manually, add it to your PATH environment variable, and restart QUAST.')
    return downloaded_fpath
...
def verify_md5(fpath, md5_fpath=None):
    ...
    if is_non_empty_file(fpath) and is_non_empty_file(md5_fpath):
        ...
        if expected_md5 != calculated_md5:
            print_warning('Failure of md5 check for %s! Expected md5 is %s, calculated md5 is %s.' %
                           (fpath, expected_md5, calculated_md5))
            return False
        return True
    print_warning('Failed to check md5 for %s! Either this file or its md5 file (%s) is missing or empty.' %
                   (fpath, md5_fpath))
    return False
