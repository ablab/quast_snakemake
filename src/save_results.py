...
from src import qconfig, qutils, reporting, plotter
from src.analyze_misassemblies import Misassembly
...
def save_result(result, report, label, ref_fpath, genome_size):
    region_misassemblies = result['region_misassemblies']
    misassemblies_by_ref = dict((k,v) for k, v in zip(result['misassemblies_by_ref'], result['misassemblies_by_ref_bases']))
    misassembled_contigs = result['misassembled_contigs']
    ...
    SNPs = result['SNPs']
    indels_list = result['indels_list'] if 'indels_list' in result else []
    ...
    if qconfig.is_combined_ref:
        ...
        for ref_name in all_references:
            subreport = reporting.get(label, ref_name=ref_name)
            ...
            if label not in qconfig.dict_of_broken_scaffolds:
