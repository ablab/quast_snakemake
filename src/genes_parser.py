...
from src.common import open_gzipsafe
from src.logger import print_warning
...
def get_genes_from_file(fpath, feature):
    ...
    genes_file = open_gzipsafe(fpath)
    genes = []
    ...
    if fpath.endswith('bed') or fpath.endswith('bed.gz'):
        ...
        try:
            ...
            print_warning('Parsing exception ' + exc_value)
            print_warning(fpath + ' was skipped')
            genes = []
    else:
        print_warning('Incorrect format of ' + feature + '\'s file! GFF, NCBI and the plain TXT format accepted. See manual.')
        print_warning(fpath + ' was skipped')
        ...
        for info_line in the_rest_lines:
            ...
            if info_line.startswith('Annotation:'):
                ...
                if m:
                    ...
                    print_warning('Wrong NCBI annotation for gene ' + str(gene.number) + '. ' + gene.name + '. Skipping this gene.')
            ...
            if info_line.startswith('ID:'):
                ...
                if m:
                    ...
                    print_warning('Can\'t parse gene\'s ID in NCBI format. Gene is ' + str(gene.number) + '. ' + gene.name + '. Skipping it.')
...
class Gene():
    ...
    def asdict(self):
        return {'id': self.id, 'seqname': self.seqname, 'start': self.start, 'end': self.end, 'number': self.number,
                'name': self.name, 'chromosome': self.chromosome, 'contig': self.contig, 'strand': self.strand,
                'seq': self.seq, 'protein': self.protein, 'is_full': self.is_full}
