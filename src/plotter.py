...
from src.qutils import parse_str_to_num
...
if qconfig.draw_plots:
    try:
        ...
        if matplotlib.__version__.startswith('0') or matplotlib.__version__.startswith('1.0'):
            print_info('')
            print_warning('Can\'t draw plots: matplotlib version is old! Please use matplotlib version 1.1 or higher.')
        else:
        ...
        print_info('')
        print_warning('Can\'t draw plots: python-matplotlib is missing or corrupted.')
...
def save_to_pdf(all_pdf_fpath):
    try:
        ...
        print_warning('PDF with all tables and plots cannot be created')
        return
...
def create_plot(plot_fpath, title, plots, legend_list=None, x_label=None, y_label=None, vertical_legend=False, is_histogram=False,
    ...
    print_info('    saved to ' + plot_fpath)
    if add_to_report:
...
def cumulative_plot(reference, labels, lists_of_lengths, plot_fpath, title):
    ...
    print_info('  Drawing cumulative plot...')

    plots = []
    max_x = 0

    for (label, lengths) in zip(labels, lists_of_lengths):
        y_vals = [0]
...
def frc_plot(results_dir, ref_fpath, labels, contigs_aligned_lengths, features_in_contigs_by_file, plot_fpath, title):
    ...
    for i, label in enumerate(labels):
        aligned_lengths = contigs_aligned_lengths[i]
        feature_in_contigs = features_in_contigs_by_file[label]
        if not aligned_lengths or not feature_in_contigs:
            continue

        aligned_assemblies.append(label)
    ...
    if qconfig.html_report:
        html_saver.save_coord(results_dir, json_vals_x, json_vals_y, 'coord' + title, aligned_assemblies)

    if can_draw_plots:
        title = 'FRCurve (' + title + ')'
        legend_list = aligned_assemblies
...
def Nx_plot(results_dir, reduce_points, labels, lists_of_lengths, plot_fpath, title='Nx', reference_lengths=None):
    if can_draw_plots:
        print_info('  Drawing ' + title + ' plot...')

    plots = []
    json_vals_x = []  # coordinates for Nx-like plots in HTML-report
    json_vals_y = []

    for id, (label, lengths) in enumerate(zip(labels, lists_of_lengths)):
        if not lengths:
            json_vals_x.append([])
            json_vals_y.append([])
            continue
    ...
    if qconfig.html_report:
        html_saver.save_coord(results_dir, json_vals_x, json_vals_y, 'coord' + title, labels)
...
# routine for GC-plot
def GC_content_plot(ref_fpath, labels, list_of_GC_distributions, plot_fpath):
    ...
    for i, (GC_distribution_x, GC_distribution_y) in enumerate(list_of_GC_distributions):
        ...
        # add to plot
        if ref_fpath and (i == len(all_labels) - 1):
            color = reference_color
            ls = reference_ls
        else:
            color, ls = get_color_and_ls(all_labels[i])
...
def contigs_GC_content_plot(label, GC_distributions, plot_fpath):
    if not can_draw_plots or qconfig.no_gc:
        return
    title = label + ' GC content'
    print_info('  Drawing ' + title + ' plot...')
...
def genes_operons_plot(reference_value, labels, files_feature_in_contigs, plot_fpath, title):
    if not can_draw_plots:
        return

    print_info('  Drawing ' + title + ' cumulative plot...')
    ...
    for label in labels:
        # calculate values for the plot
        feature_in_contigs = files_feature_in_contigs[label]
...
def histogram(labels, values, plot_fpath, title='', yaxis_title='', bottom_value=None,
              top_value=None):
    if not can_draw_plots:
        return
    if len(labels) < 2:  #
        print_info('  Skipping drawing ' + title + ' histogram... (less than 2 columns histogram makes no sense)')
        return

    print_info('  Drawing ' + title + ' histogram...')
    ...
    create_plot(plot_fpath, title, plots, legend_list, x_label='', y_label=yaxis_title, is_histogram=True,
                x_limit=[0, start_pos + width * len(labels) + interval * (len(labels) - 1)],
                y_limit=[max(bottom_value, 0), top_value])


def coverage_histogram(labels, values, plot_fpath, title='', bin_size=None, draw_bars=None, max_cov=None,
                       low_threshold=None, high_threshold=None):
    if not can_draw_plots:
        return

    print_info('  Drawing ' + title + '...')
    ...
    for i, (label, y_vals) in enumerate(zip(labels, values)):
...
def draw_meta_summary_plot(html_fpath, output_dirpath, labels, ref_names, results, plot_fpath, title='', reverse=False,
    ...
    if can_draw_plots:
        print_info('  Drawing ' + title + ' metaQUAST summary plot...')
    ...
    for j in range(contigs_num):
        ...
        color, ls = get_color_and_ls(labels[j])
...
def draw_meta_summary_misassemblies_plot(results, ref_names, contig_num, plot_fpath, title=''):
    if can_draw_plots:
        print_info('  Drawing metaQUAST summary misassemblies plot for ' + title + '...')
...
def draw_misassemblies_plot(reports, plot_fpath, title='', yaxis_title=''):
    ...
    print_info('  Drawing misassemblies by types plot...')
