...
from src.analyze_misassemblies import Mapping
from src.logger import print_info
from src.misc import *
from src.qutils import is_non_empty_file
...
def check_successful_check(fpath, contigs_fpath, ref_fpath):
    ...
    if len(successful_check_content) < 2:
        return False
    if successful_check_content[0].strip().split()[-1] != str(qutils.md5(contigs_fpath)):
        return False
    if successful_check_content[1].strip().split()[-1] != str(qutils.md5(ref_fpath)):
        return False
    return True


def run_minimap_agb(out_fpath, ref_fpath, contigs_fpath, log_err_fpath, max_threads):  # run minimap2 for AGB
    mask_level = '1' if qconfig.min_IDY < 95 else '0.9'
    cmdline = ['minimap2', '-cx', 'asm20', '--mask-level', mask_level, '-N', '100',
...
def run_minimap(out_fpath, ref_fpath, contigs_fpath, log_err_fpath, max_threads):
    if qconfig.is_agb_mode:
        return run_minimap_agb(out_fpath, ref_fpath, contigs_fpath, log_err_fpath, max_threads)
    ...
    cmdline = ['minimap2', '-c', '-x', preset] + (additional_options if not qconfig.large_genome else []) + \
...
def split_align(coords_file, align_start, strand_direction, ref_start, ref_name, contig, cs):
    ...
    def _try_split(matched_bases, prev_op, n_refbases=0, n_alignbases=0):
        ...
        if n_alignbases > qconfig.SPLIT_ALIGN_THRESHOLD or n_refbases > qconfig.SPLIT_ALIGN_THRESHOLD:
            _write_align()
...
def align_contigs(output_fpath, out_basename, ref_fpath, contigs_fpath, threads, log_out_fpath, log_err_fpath):
    ...
    if qconfig.alignments_for_reuse_dirpath is not None and os.path.isdir(qconfig.alignments_for_reuse_dirpath):
        ...
        if isfile(coords_to_reuse_fpath):
            ...
            print_info('  ' + 'Reusing alignments from the combined_reference stage... ')
    ...
    if isfile(successful_check_fpath) and isfile(output_fpath):
        if check_successful_check(successful_check_fpath, contigs_fpath, ref_fpath):
            log_out_f.write('\tUsing existing alignments...\n')
            print_info('  ' + 'Using existing alignments... ')
            return AlignerStatus.OK

    log_out_f.write('\tAligning contigs to the reference\n')
    print_info('  ' + 'Aligning contigs to the reference')

    tmp_output_fpath = output_fpath + '_tmp'
    exit_code = run_minimap(tmp_output_fpath, ref_fpath, contigs_fpath, log_err_fpath, threads)
    if exit_code != 0:
    ...
    create_successful_check(successful_check_fpath, contigs_fpath, ref_fpath)
