...
from src import qconfig, plotter, qutils
...
assembly_labels = []  # for printing in appropriate order
...
def get(assembly_label, ref_name=None):
    if not ref_name and qconfig.reference:
        ref_name = qutils.name_from_fpath(qconfig.reference)
    if assembly_label not in assembly_labels:
        assembly_labels.append(assembly_label)
    return reports.setdefault((os.path.abspath(assembly_label), ref_name), Report(assembly_label))


def delete(assembly_label):
    if assembly_label in assembly_labels:
        assembly_labels.remove(assembly_label)
    if assembly_label in reports.keys():
        reports.pop(assembly_label)


# ATTENTION! Contents numeric values, needed to be converted into strings
def table(reports, order=Fields.order, ref_name=None):
...
def save(reports, output_dirpath, report_name, transposed_report_name, order, silent=False):
    # Where total report will be saved
    tab = table(reports, order)

    if not silent:
        print_info('  Creating total report...')
    ...
    if not silent:
        print_info('    saved to ' + reports_fpaths)

    if transposed_report_name:
        if not silent:
            print_info('  Transposed version of total report...')

        all_rows = get_all_rows_out_of_table(tab)
        if all_rows[0]['metricName'] != Fields.NAME:
            print_warning('transposed version can\'t be created! First column have to be assemblies names')
        else:
            # Transposing table
            ...
            if not silent:
                print_info('    saved to ' + transposed_reports_fpaths)
    return reports_fpaths, transposed_reports_fpaths


def save_total(reports, output_dirpath, silent=True):
    if not silent:
        print_timestamp()
        print_info('Summarizing...')
    return save(reports, output_dirpath, qconfig.report_prefix, qconfig.transposed_report_prefix, Fields.order, silent=silent)


def save_misassemblies(reports, output_dirpath):
    save(reports, output_dirpath, "misassemblies_report", qconfig.transposed_report_prefix + "_misassemblies", Fields.misassemblies_order_advanced)


def save_unaligned(reports, output_dirpath):
    save(reports, output_dirpath, "unaligned_report", "", Fields.unaligned_order)


def save_reads(reports, output_dirpath):
    save(reports, output_dirpath, "reads_report", "", Fields.reads_order)


def save_kmers(reports, output_dirpath):
    save(reports, output_dirpath, "kmers_report", "", Fields.kmers_order)