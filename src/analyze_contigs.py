...
from src.best_set_selection import *
...
def analyze_contigs(ca_output, contigs_fpath, unaligned_fpath, unaligned_info_fpath, aligns, ref_features, ref_lens,
    ...
    for record in read_fasta(contigs_fpath):
        ...
        filtered_aligns = []
        if record.id in aligns:
            filtered_aligns = [align for align in aligns[record.id] if align.len2 >= qconfig.min_alignment]
        ...
        if filtered_aligns:
            ...
            if top_len > ctg_len * epsilon or ctg_len - top_len < maxun:
                ...
                if len(real_aligns) == 1:
                    ...
                    if is_partially_unaligned:
                        ...
                        save_unaligned_info(real_aligns, record.id, ctg_len, unaligned_bases, unaligned_info_file)
                    ca_output.stdout_f.write('\t\tAlignment: %s\n' % str(the_only_align))
                    ...
                    if is_partially_unaligned:
                        ...
                        ca_output.stdout_f.write('\t\tThis contig is partially unaligned. '
                                                 ...
                                                 % (aligned_bases_in_contig, acgt_ctg_len,
                                                 100.0 * aligned_bases_in_contig / acgt_ctg_len))
                        save_unaligned_info(sorted_aligns, record.id, ctg_len, unaligned_bases, unaligned_info_file)
                    ...
                    if aligned_bases_in_contig < qconfig.unaligned_mis_threshold * acgt_ctg_len:
                        ...
                        ca_output.icarus_out_f.write('\t'.join(['CONTIG', record.id, str(ctg_len), contig_type + '\n']))
                    ...
                    if is_misassembled:
                        misassembled_contigs[record.id] = ctg_len
            ...
            unaligned_file.write(record.id + '\n')
            ...
            save_unaligned_info([], record.id, ctg_len, ctg_len, unaligned_info_file)
    ...
    misassembled_contigs_names = list(misassembled_contigs.keys())
    misassemblies_by_ref_names = list(misassemblies_by_ref.keys())
    ...
    result = {'region_misassemblies': region_misassemblies,
              ...
              'misassemblies_by_ref': misassemblies_by_ref_names,
              'misassemblies_by_ref_bases': [misassemblies_by_ref[c] for c in misassemblies_by_ref_names],
