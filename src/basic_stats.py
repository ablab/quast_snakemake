...
def GC_content(contigs_fpath, skip=False):
    ...
    for record in read_fasta(contigs_fpath):
        contig_ACGT_len = len(record.seq) - record.seq.count("N")
        if not contig_ACGT_len:
            continue
        contig_GC_len = record.seq.count("G") + record.seq.count("C")
        ...
        for seq in (record.seq[i:i+n] for i in range(0, len(record.seq), n)):
...
def save_icarus_GC(ref_fpath, gc_fpath):
    ...
    with open(gc_fpath, 'w') as out_f:
        for record in read_fasta(ref_fpath):
            out_f.write('#' + record.id + ' ' + str(chr_index) + '\n')
    ...
    with open(gc_fpath, 'w') as out_f:
        for record in read_fasta(ref_fpath):
            for i in range(0, len(record.seq), window_size):
                ...
                GC_percent = get_GC_percent(seq)
                if GC_percent is not None:
                    out_f.write('\t'.join([record.id, str(i), str(i + len(seq)), str(GC_percent) + '\n']))
...
def draw_coverage_histograms(coverage_dict, labels, reports, output_dirpath):
    total_len = dict()
    contigs_dict = dict()

    contigs_with_coverage = [label for label in labels if coverage_dict[label]]
    for label in labels:
        total_len[label] = reports[label].get_field(reporting.Fields.TOTALLEN)
        contigs_dict[label] = reports[label].get_field(reporting.Fields.CONTIGS)
    cov_values = [coverage_dict[label] for label in contigs_with_coverage]
    num_contigs = [contigs_dict[label] for label in contigs_with_coverage]
    ...
    for label in contigs_with_coverage:
        coverage_values, bin_size, low_threshold, high_threshold, max_cov = binning_coverage([coverage_dict[label]],
                                                                                             [contigs_dict[label]])

        histogram_title = label + ' coverage histogram (bin size: ' + str(bin_size) + 'x)'
        histogram_fpath = os.path.join(output_dirpath, label + '_coverage_histogram')
        plotter.coverage_histogram([label], coverage_values, histogram_fpath,
...
def do(reports, ref_fpath, reference_chromosomes, assemblies, results_dir, output_dirpath):
    ...
    if ref_fpath:
        reference_lengths = sorted(get_chr_lengths_from_fastafile(ref_fpath).values(), reverse=True)
        ...
        print_info('  Reference genome:')
        print_info('    ' + os.path.basename(ref_fpath) + ', length = ' + str(reference_length) +
        ...
        if reference_fragments > 30 and not qconfig.check_for_fragmented_ref:
            print_warning('  Reference genome is fragmented. You may consider rerunning QUAST using --fragmented option.'
        ...
        print_info('  Estimated reference length = ' + str(reference_length))

    print_info('  Contig files: ')
    lists_of_lengths = []
    numbers_of_Ns = []
    coverage_dict = dict()
    cov_pattern = re.compile(r'_cov_(\d+\.?\d*)')
    for id, assembly in enumerate(assemblies):
        coverage_dict[assembly.label] = []
        print_info('    ' + assembly.label)
        # lists_of_lengths.append(fastaparser.get_lengths_from_fastafile(contigs_fpath))
        list_of_length = []
        number_of_Ns = 0
        for record in read_fasta(assembly.contigs_fpath):
            list_of_length.append(len(record.seq))
            number_of_Ns += record.seq.count('N')
            if cov_pattern.findall(record.id):
                cov = int(float(cov_pattern.findall(record.id)[0]))
                if len(coverage_dict[assembly.label]) <= cov:
                    coverage_dict[assembly.label] += [0] * (cov - len(coverage_dict[assembly.label]) + 1)
                coverage_dict[assembly.label][cov] += len(record.seq)
    ...
    labels = [asm.label for asm in assemblies]
    if qconfig.html_report:
        html_saver.save_contigs_lengths(results_dir, labels, corr_lists_of_lengths)
        html_saver.save_tick_x(results_dir, multiplicator)

    ########################################################################

    print_info('  Calculating N50 and L50...')

    list_of_GC_distributions = []
    list_of_GC_contigs_distributions = []
    largest_contig = 0
    from . import N50
    for id, (assembly, lengths_list, number_of_Ns) in enumerate(zip(assemblies, lists_of_lengths, numbers_of_Ns)):
        report = reports[assembly.label]
        ...
        total_GC, GC_distribution, GC_contigs_distribution = GC_content(assembly.contigs_fpath, skip=qconfig.no_gc)
    ...
    if qconfig.html_report and not qconfig.no_gc:
        html_saver.save_GC_info(results_dir, labels, list_of_GC_distributions_with_ref, list_of_GC_contigs_distributions, reference_index)

    ########################################################################
    # Drawing Nx and NGx plots...
    plotter.Nx_plot(results_dir, num_contigs > qconfig.max_points, labels, lists_of_lengths, join(output_dirpath, 'Nx_plot'), 'Nx', [])
    if reference_length and not qconfig.is_combined_ref:
        plotter.Nx_plot(results_dir, num_contigs > qconfig.max_points, labels, lists_of_lengths, join(output_dirpath, 'NGx_plot'), 'NGx',
                        [reference_length for i in range(len(labels))])
    ...
    if qconfig.draw_plots:
        ...
        if not qconfig.no_gc:
            ...
            plotter.GC_content_plot(ref_fpath, labels, list_of_GC_distributions_with_ref, join(output_dirpath, 'GC_content_plot'))
            for label, GC_distribution in zip(labels, list_of_GC_contigs_distributions):
                plotter.contigs_GC_content_plot(label, GC_distribution,
                                                join(output_dirpath, label + '_GC_content_plot'))

        if any(coverage_dict[label] for label in labels):
            draw_coverage_histograms(coverage_dict, labels, reports, output_dirpath)
