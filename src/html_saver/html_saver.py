...
from src.html_saver import json_saver
...
def save_icarus_html(template_fpath, html_fpath, data_dict):
    ...
    html = template(html, **data_dict, template_settings={'noescape': True})
    #html = .expand(html, data_dict, more_formatters={
    #    'join': lambda v: ', '.join(v),
    #})
    #Template(html).substitute(data_dict)
...
def save_total_report(reports, results_dirpath, labels, min_contig, ref_fpath):
    json_fpath = json_saver.save_total_report(reports, results_dirpath, labels, min_contig, ref_fpath)
    if json_fpath:
        json_saver.json_text = append(results_dirpath, json_fpath, 'totalReport')
        print_info('  HTML version (interactive tables and plots) is saved to ' + os.path.join(results_dirpath, report_fname))
...
def save_coord(results_dirpath, coord_x, coord_y, name_coord, labels):  # coordinates for Nx, NAx, NGx, NGAX
    json_fpath = json_saver.save_coord(results_dirpath, coord_x, coord_y, name_coord, labels)
    if json_fpath:
        append(results_dirpath, json_fpath, name_coord)


def save_colors(results_dirpath, labels, dict_colors, meta=False):  # coordinates for Nx, NAx, NGx, NGAX
    if meta:
        html_fpath = os.path.join(results_dirpath, report_fname)
        with open(html_fpath) as f_html:
            html_text = f_html.read()
        ...
        colors_and_ls = [dict_colors[contig_label] for contig_label in labels]
        colors = [color_and_ls[0] for color_and_ls in colors_and_ls]
        colors_for_html = [html_colors[colors.index(color)] for color in colors]
        save_record(results_dirpath, 'colors', colors_for_html)
        broken_contig_names = [label for i, label in enumerate(labels) if colors_and_ls[i][1] == secondary_line_style]
...
def save_contigs_lengths(results_dirpath, labels, lists_of_lengths):
    json_fpath = json_saver.save_contigs_lengths(results_dirpath, labels, lists_of_lengths)
    if json_fpath:
        append(results_dirpath, json_fpath, 'contigsLengths')


def save_assembly_lengths(results_dirpath, labels, assemblies_lengths):
    json_fpath = json_saver.save_assembly_lengths(results_dirpath, labels, assemblies_lengths)
    if json_fpath:
        append(results_dirpath, json_fpath, 'assembliesLengths')


def save_features_in_contigs(results_dirpath, labels, feature_name, feature_in_contigs, ref_feature_num):
    json_fpath = json_saver.save_features_in_contigs(results_dirpath, labels, feature_name, feature_in_contigs, ref_feature_num)
    if json_fpath:
        append(results_dirpath, json_fpath, feature_name + 'InContigs')


def save_GC_info(results_dirpath, labels, list_of_GC_distributions, list_of_GC_contigs_distributions, reference_index):
    json_fpath = json_saver.save_GC_info(results_dirpath, labels, list_of_GC_distributions, list_of_GC_contigs_distributions, reference_index)
    if json_fpath:
