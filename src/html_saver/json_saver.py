...
from src import qutils, qconfig, reporting
...
try:
    ...
    try:
        ...
        print_warning('Can\'t build html report - please install python-simplejson')
...
def save_total_report(reports, output_dirpath, labels, min_contig, ref_fpath):
    asm_names = labels
    report = reporting.table(reports, reporting.Fields.grouped_order)
    subreports = []
    ref_names = []
...
def save_contigs_lengths(output_dirpath, labels, lists_of_lengths):
...
def save_coord(output_dirpath, coord_x, coord_y, name_coord, labels):
...
def save_assembly_lengths(output_dirpath, labels, assemblies_lengths):
...
def save_features_in_contigs(output_dirpath, labels, feature_name, features_in_contigs, ref_features_num):
    return save(join(output_dirpath, feature_name + in_contigs_suffix_fn), {
        'filenames': labels,
        feature_name + '_in_contigs': dict((label, feature_amounts)
                                           for (label, feature_amounts) in features_in_contigs.items()),
        'ref_' + feature_name + '_number': ref_features_num,
    })


def save_GC_info(output_dirpath, labels, list_of_GC_distributions, list_of_GC_contigs_distributions, reference_index):
